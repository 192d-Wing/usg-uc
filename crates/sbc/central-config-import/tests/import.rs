...
async fn imports_live_rows_into_central_shard() {
    ...
    sbc_config_store::ensure_schema(&site_db)
        .await
        .expect("local schema");
    sqlx::query("INSERT INTO phones (id, mac_normalized, data) VALUES ($1,$2,$3)")
        .bind("p1")
        .bind("aabbccddeeff")
        .bind(json!({"id":"p1","mac_address":"aa:bb:cc:dd:ee:ff"}))
        .execute(&site_db)
        .await
        .expect("phone");
    ...
    sqlx::query("INSERT INTO trunk_groups (id, data) VALUES ($1,$2)")
        .bind("us-domestic")
        .bind(json!({"id":"us-domestic","strategy":"priority"}))
        .execute(&site_db)
        .await
        .expect("tg");
    ...
    let central = CentralConfigStore::from_pool(make_db(&admin, "cci_central").await)
        .await
        .expect("central");
    central
    ...
    let report = import_site(&site_db, &central, "MUHJ", "importer")
        .await
        .expect("import");
    ...
    let owner: Option<String> =
        sqlx::query_scalar("SELECT site_code FROM did_registry WHERE did='5551234567'")
            .fetch_optional(central.pool())
            .await
            .expect("registry");
    assert_eq!(owner.as_deref(), Some("MUHJ"));

    // Idempotent: a second import re-upserts without error.
    let report2 = import_site(&site_db, &central, "MUHJ", "importer")
        .await
        .expect("re-import");
    assert_eq!(report2.phones, 1);
    assert_eq!(
        count(ConfigTable::Phones),
        1,
        "still one phone after re-import"
    );
}
