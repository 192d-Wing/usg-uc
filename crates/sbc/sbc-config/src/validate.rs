...
mod tests {
    ...
    #[test]
    fn test_stir_shaken_verification_defaults_off() {
        // The daemon has no verification code path today; defaulting
        // verification to "on" would be a false sense of security.
        let config = SbcConfig::default();
        assert!(!config.stir_shaken.verification_enabled);
    }
}
