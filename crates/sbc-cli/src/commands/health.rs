...
use sbc_health::{HealthChecker, HealthCheckerConfig, HealthStatus};
...
pub fn run(args: &Args) -> CommandResult {
    ...
    // Add checks
    checker.register(Box::new(sbc_health::check::AlwaysHealthyCheck::new("sbc_core")));
    checker.register(Box::new(sbc_health::check::MemoryCheck::new()));
    checker.register(Box::new(sbc_health::check::DiskCheck::new("/")));
