...
use sbc_crypto::aead::Aes256GcmKey;
use sbc_crypto::ecdh::P384EphemeralKeyPair;
use sbc_crypto::hkdf;
use std::sync::Arc;
...
impl Handshake {
    ...
    pub fn new(
        ...
        if is_client {
            sbc_crypto::random::fill_random(&mut client_random).map_err(|e| {
                DtlsError::HandshakeFailed {
                    reason: format!("failed to generate client random: {e}"),
                }
            })?;
        } else {
            sbc_crypto::random::fill_random(&mut server_random).map_err(|e| {
                DtlsError::HandshakeFailed {
    ...
    fn build_hello_verify_request(&mut self) -> DtlsResult<Vec<u8>> {
        ...
        sbc_crypto::random::fill_random(&mut cookie).map_err(|e| {
            DtlsError::HandshakeFailed {
    ...
    fn build_finished(&self) -> DtlsResult<Vec<u8>> {
        ...
        let handshake_hash = sbc_crypto::hash::sha384(&self.handshake_hash);
