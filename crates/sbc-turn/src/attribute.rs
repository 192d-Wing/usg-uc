...
fn parse_xor_address(data: &[u8], transaction_id: &[u8; 12]) -> TurnResult<SocketAddr> {
    ...
    let port = x_port ^ ((sbc_stun::MAGIC_COOKIE >> 16) as u16);
    ...
    let addr = match family {
        0x01 => {
            ...
            let addr = x_addr ^ sbc_stun::MAGIC_COOKIE;
        ...
        0x02 => {
            ...
            let cookie_bytes = sbc_stun::MAGIC_COOKIE.to_be_bytes();
            for i in 0..4 {
...
fn encode_xor_address(addr: &SocketAddr, transaction_id: &[u8; 12]) -> Bytes {
    ...
    let x_port = addr.port() ^ ((sbc_stun::MAGIC_COOKIE >> 16) as u16);

    match addr {
        SocketAddr::V4(v4) => {
            buf.put_u8(0x01); // IPv4 family
            buf.put_u16(x_port);
            let x_addr = u32::from_be_bytes(v4.ip().octets()) ^ sbc_stun::MAGIC_COOKIE;
            buf.put_u32(x_addr);
        }
        SocketAddr::V6(v6) => {
