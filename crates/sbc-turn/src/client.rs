...
//!
//! ```ignore
//! use sbc_turn::client::TurnClient;
//!
...
use sbc_stun::message::{StunClass, StunMessage, StunMessageType, StunMethod};
use sbc_stun::StunAttribute;
...
impl TurnClient {
    ...
    fn create_allocate_request(&self) -> TurnResult<StunMessage> {
        ...
        sbc_crypto::random::fill_random(&mut transaction_id).map_err(|_| {
    ...
    async fn send_request(&self, request: &StunMessage) -> TurnResult<StunMessage> {
        ...
        while attempts <= self.max_retries {
            if start.elapsed() > self.timeout {
                return Err(TurnError::Stun(sbc_stun::StunError::Timeout));
            }
        ...
        Err(TurnError::Stun(sbc_stun::StunError::Timeout))
    }
    ...
    async fn recv_response(&self, expected_tid: &[u8; 12]) -> TurnResult<StunMessage> {
        ...
        loop {
            let (n, from) = self.socket.recv_from(&mut buf).await.map_err(|e| {
                TurnError::Stun(sbc_stun::StunError::NetworkError {
